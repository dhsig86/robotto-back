// Extração local (fallback) baseada em aliases e heurísticas simples.
...
export function fallbackExtract({ text, registry, allowed }) {
  ...
  // Idade (heurística simples: número seguido de 'a', 'anos', 'ano')
  ...
  // Mapeamento por aliases (derivado do registry)
  for (const [aliasNorm, fid] of registry.aliasToId.entries()) {
    if (!allowed.has(fid)) continue;
    // match direto por substring normalizada
    if (norm.includes(aliasNorm)) features.add(fid);
  }

  // Heurística: tokens do id (com underscores) presentes
